...
//CCU10 T12 INTERRRUPT
        ...
        SRC_CCU60_SR0   &= ~ ((0xFF) << SRPN); //인터럽트 우선순위 초기화
        SRC_CCU60_SR1   &= ~ ((0xFF) << SRPN);

        SRC_CCU60_SR0   |=   ((0x0B) << SRPN); //0B 입력 인터럽트 우선순위
        SRC_CCU60_SR1   |=   ((0x0A) << SRPN); //0A 입력

        SRC_CCU60_SR0   &= ~ ((0x3) << TOS);   //CPU 위치
        SRC_CCU60_SR1   &= ~ ((0x3) << TOS);   //CPU 위치
...
__interrupt(0x0A)__vector_table(0) //깜빡이는 LED
...
__interrupt(0x0B)__vector_table(0) //깜빡이는 LED
...
__interrupt(0x10)__vector_table(0) // 스위치
void ERU0_ISR(void)
{
    PORT10_OMR |= ((1<<PCL1) | (1<<PS1));
}
...
__interrupt(0x09)__vector_table(0) //스위치
void ERU1_ISR(void)
{
    PORT10_OMR |= ((1<<PCL2) | (1<<PS2));
}
